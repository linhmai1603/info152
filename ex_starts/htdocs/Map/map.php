<?php
  ...
  $apikey = "AIzaSyDk_HIWuKjvc8qd2u6s2QG3qRFZMsWaZj8";
  $id = $_GET['id'];
      ...
      function initialize() {
        ...
        var map = new google.maps.Map(document.getElementById("map-canvas"),
			...
			<?php
				...
				$query($getpoints){die(‘There was an error running the query [' . $con->error . ']‘);
                   }else {
							while ($row = $result->fetch_assoc()) {
							echo "var myLatlng1 = new google.maps.LatLng($row[pointLat], $row[pointLong]);
							var marker1 = new google.maps.Marker({
							position: myLatlng1,map: map,title: ‘$row[pointText]‘});";
							}
						}
